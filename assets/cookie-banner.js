(function () {
  ...
  function loadConsentContent(allowExternal = false) {
    ...
    document.querySelectorAll(".consent-placeholder").forEach(container => {
      ...
      if (type === "iframe" && mobileLink) {
        container.innerHTML = `
    <div class="rr-mobile-box">
      <p>Die Anmeldung wird in einem neuen Tab geöffnet.</p>
      <a href="${mobileLink}" target="_blank" rel="noopener" class="rr-btn">
        Anmeldung öffnen
      </a>
    </div>
  `;
      ...
      // Fallback (falls zukünftig wieder iframes gewünscht: Code aus alter Version verwenden)
      container.innerHTML = "";
    });
    // --- ENDE NEUE VERSION ---
  }
